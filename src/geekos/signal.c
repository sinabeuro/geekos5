...
void Complete_Handler(struct Kernel_Thread *kthread, struct Interrupt_State* esp)
{
	...
	memcpy(esp, Get_User_Address((ulong_t)p[0]), sizeof(struct Interrupt_State)+2*sizeof(uint_t));
...
{	
	...
	/* point first of user stack location */
	p = (void **)(((struct Interrupt_State *)esp)+1);
	userStackPtr = (char*)Get_User_Address((ulong_t)p[0]);
	userStackDs = (char*)((unsigned int)p[1]);

	/* backup the user stack */
	/* data selector */
	userStackPtr -= sizeof(ulong_t);
	*((ulong_t *) userStackPtr) = (ulong_t)userStackDs;

	/* top of user stack pointer */
	userStackPtr -= sizeof(ulong_t);
	*((ulong_t *) userStackPtr) = (ulong_t)p[0];

	/* interrupt states */
	userStackPtr -= sizeof(struct Interrupt_State);
	memcpy(userStackPtr, esp, sizeof(struct Interrupt_State));

	/* for return at return signal system call after run signal handler */
	userStackPtr -= sizeof(signal_handler*);
	*((signal_handler *) userStackPtr) = kthread->userContext->returnSignal;

	/* program counter must point signal handler */
	esp->eip = kthread->userContext->saHandler[kthread->userContext->signal];
	*((ulong_t *)&p[0]) = Get_Kernel_Address(userStackPtr);

	/* initial signal context */
